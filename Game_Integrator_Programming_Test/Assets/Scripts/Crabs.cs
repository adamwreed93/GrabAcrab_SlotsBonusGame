...
public class Crabs : MonoBehaviour
{
    ...
    {
        if (_canSelectCrab == true && _numberOfCrabsSelected < uiManager._currentWinningsSequence.Length -1 && _isGameOver == false)
        {
    ...
    {
        if (_numberOfCrabsSelected < uiManager._currentWinningsSequence.Length - 1)
        {
            _nextAmmountInSequence = uiManager._currentWinningsSequence[_numberOfCrabsSelected];
            _winningsAmmountText.text = "$" + _nextAmmountInSequence + ".00";
        }
        else if (_numberOfCrabsSelected == uiManager._currentWinningsSequence.Length - 1)
        {
            _nextAmmountInSequence = uiManager._currentWinningsSequence[_numberOfCrabsSelected];
            _winningsAmmountText.text = "$" + _nextAmmountInSequence + ".00";
